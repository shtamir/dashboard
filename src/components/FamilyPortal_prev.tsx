...
useEffect(() => {
    ...
    fetchCalendarEvents()
      .then((result) => {
        console.log('✔ got', result.events.length, 'events from fetch');
        console.log('🔄 updating with', result.events.length, 'items');
  ...
  const getDateForViewMode = (dayIndex: number, viewModeIndex: number) => {
    const today = new Date();
    const currentDayIndex = today.getDay();
    ...
    return (
      <div className={`grid gap-2`} style={{gridTemplateColumns: `repeat(${getDaysToShow().length}, minmax(0, 1fr))`}}>
        {dayOffsets.map((offset) => {
          const dayEvents = calendarEvents.filter((e) => e.day === offset);
          const isToday = new Date().getDay() === offset;
          
          return (
            <div key={`events-${offset}`} className={`bg-white dark:bg-gray-800 rounded-b-2xl ${settings.compactMode ? 'p-3 min-h-80' : 'p-4 min-h-96'} border-4 border-t-0 ${isToday ? 'border-blue-500' : 'border-transparent'}`}>
